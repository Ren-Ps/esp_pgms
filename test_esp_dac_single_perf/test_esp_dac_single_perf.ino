...
void setup() 
{
  ...
  dac_cosine_enable(DAC_CHANNEL_1); //dac_cosine_enable(DAC_CHANNEL_2);

  dac_output_enable(DAC_CHANNEL_1); //dac_output_enable(DAC_CHANNEL_2);
}

void loop(){ 
        dac_frequency_set(clk_8m_div, frequency_step);// frequency setting is common to both channels
        dac_scale_set(DAC_CHANNEL_1, scale);  // Tune parameters of channel 1 only
        dac_offset_set(DAC_CHANNEL_1, offset);
        dac_invert_set(DAC_CHANNEL_1, invert);
        dac_scale_set(DAC_CHANNEL_2, scale);  // Tune parameters of channel 2 only
